...
def validate_assumptions(
    ...
    if assumptions.investor_capital <= 0:
        errors.append(
            "Investor capital must be greater than zero."
        )
    ...
    if not 0 <= assumptions.investor_return_target <= 1:
        errors.append(
            "Investor return target must be between 0% and 100%."
        )

    if assumptions.exit_multiple <= 0:
        errors.append(
            "Exit multiple must be greater than zero."
        )
    ...
    if (
        assumptions.financing_type == "Equity"
        and assumptions.ownership_percentage <= 0
    ):
        errors.append(
